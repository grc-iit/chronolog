...
int ChronicleMetaDirectory::release_story(chl::ClientId const& client_id,
                                          ...
                                          const std::string& story_name,
                                          StoryId& story_id,
                                          uint64_t* remaining_acquisition_count)
{
    ...
    if(chronicleRecord != chronicleMap_->end())
    {
        ...
        if(acquirerMapRecord != acquirerMap.end())
        {
            ...
            uint64_t const acquisition_count = pStory->decrementAcquisitionCount();
            if(remaining_acquisition_count != nullptr)
            {
                *remaining_acquisition_count = acquisition_count;
            }
            story_id = pStory->getSid();
