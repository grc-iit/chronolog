...
- 1-node Kafka rows are generated by colocating ZooKeeper and a single broker on the single selected node. Append/range smoke validation and high-volume 1KiB requested-grid rows now exist; remaining payload sizes still need staged execution.
