...
Chronicle-level attributes (indexing granularity, type, tiering policy, user-defined properties and metadata) are reserved for future use. The v3.0.0 client API does not expose them — `Client::CreateChronicle` takes only the chronicle name.
...
An Archive provides historical storage associated with a Chronicle. Each Archive is identified by an `ArchiveId`, computed as `CityHash64(toString(chronicleId) + archiveName)`. Archives reserve a user-defined property list for application-specific metadata; it is not exposed by the v3.0.0 client API.
