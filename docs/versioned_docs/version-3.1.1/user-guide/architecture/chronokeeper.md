...
| **Keeper Recording Service**    | Listens to incoming streams of log events from client applications (`record_event`) and serves on-demand tail reads directly from keeper memory via `tail_get_sequences` and `tail_get_events` RPCs.                                                                                                                                                                                                                                            |
| **Keeper Tail Store (`KeeperTailStore`)** | Maintains an in-memory index of recent sealed-but-not-yet-extracted `StoryChunk` pointers per story. Serves low-latency `playback(N)` queries directly from memory with zero payload copy. Manages chunk retention, capacity bounds (`tail_capacity`), time-based age-out (`tail_retention_secs`), and pin protection (`kPinTicks`). Optionally unions with active unsealed chunks when `live_tail_read` is enabled.                              |
...
| **StoryChunk Extraction Queue** | A mutex-protected deque of StoryChunk pointers, serving as the communication boundary between the Keeper Data Store / Tail Store and the ExtractionModule.                                                                                                                                                                                                                                                                                      |
...
For full architectural details on zero-copy indexing, pinning, capacity bounds, and live unsealed reads, see [On-Demand Tail Read Architecture](./on-demand-tail-read.md).
