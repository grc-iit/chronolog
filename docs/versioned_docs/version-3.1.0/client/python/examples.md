...
return_tuple = client.AcquireStory("py_chronicle", "my_story")
...
# Replay events within the half-open range [start, end) (uint64_t nanosecond timestamps)
...
## Tail Reader Client (`playback`)

This example demonstrates on-demand tail reading (`StoryHandle.playback`) directly from ChronoKeeper in-memory buffers. It corresponds to `tail_reader_client.py` in the repository.
...
# Create ClientPortalServiceConf (writer mode is sufficient for tail reads)
client_conf = py_chronolog_client.ClientPortalServiceConf("ofi+sockets", "127.0.0.1", 5555, 55)
...
client.CreateChronicle("py_chronicle")
...
if return_tuple[0] == 0:
    ...
    # Log 30 events
    event_count = 30
    print(f"\nLogging {event_count} events for tail_story...")
    for i in range(event_count):
        story_handle.log_event(f"py_tail_event.{i}")

    # Play back the most recent 10 events directly from keeper memory
    ...
    print(f"Playback returned {len(events)} event(s):")
    for i, event in enumerate(events):
        print(f"    [{i}] time={event.time()} : {event.log_record()}")

    # Asking for more than is available returns everything currently in the tail
    all_events = py_chronolog_client.EventList()
    return_code = story_handle.playback(1000, all_events)
    print(f"\nstory_handle.playback(1000, events) returned {len(all_events)} event(s)")

client.ReleaseStory("py_chronicle", "tail_story")
client.Disconnect()
```
