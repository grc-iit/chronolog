...
int main(int argc, char** argv)
{
    ...
    auto [acquire_ret, story_handle] = client.AcquireStory(chronicle_name, story_name);
...
## On-Demand Tail Read Example (`playback`)

This example demonstrates how to perform millisecond-scale on-demand tail reads of the most recent $N$ events directly from ChronoKeeper in-memory buffers using `StoryHandle::playback`.
...
{
    chronolog::ClientConfiguration confManager;
    // Writer-mode configuration is sufficient; no QueryServiceConf needed
    chronolog::Client client(confManager.PORTAL_CONF);

    if(client.Connect() != chronolog::CL_SUCCESS)
    {
        std::cerr << "Failed to connect to ChronoVisor\n";
        return 1;
    }
    ...
    auto [rc, story_handle] = client.AcquireStory(chronicle_name, story_name);
    if(rc != chronolog::CL_SUCCESS || !story_handle)
    {
        std::cerr << "Failed to acquire story\n";
        client.Disconnect();
        return 1;
    }
    ...
    for(int i = 0; i < 20; ++i)
    {
        story_handle->log_event("Sensor-reading-" + std::to_string(i));
    }
    std::cout << "Logged 20 events.\n";

    // Playback the last 5 events from keeper memory
    std::vector<chronolog::Event> tail_events;
    int playback_rc = story_handle->playback(5, tail_events);

    if(playback_rc == chronolog::CL_SUCCESS)
    {
        std::cout << "Successfully retrieved " << tail_events.size() << " tail event(s):\n";
        for(auto const& ev : tail_events)
        {
            std::cout << "  [" << ev.time() << "] " << ev.log_record() << "\n";
        }
    }
    else
    {
        std::cerr << "Playback failed: " << chronolog::to_string_client(playback_rc) << "\n";
    }
