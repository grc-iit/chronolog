...
- On-demand in-memory tail reads (`StoryHandle::playback`) for millisecond-scale retrieval of the most recent $N$ events directly from ChronoKeeper RAM.
- Full API coverage: chronicle and story lifecycle management, event logging, in-memory tail reading, and historical story replay.
...
5. **playback** — retrieve the most recent $N$ events from keeper memory (writer or reader mode).
6. **ReplayStory** — read back historical events within a time range (READER mode only).
7. **ReleaseStory** — release the story handle.
8. **DestroyStory / DestroyChronicle** — clean up resources when no longer needed.
9. **Disconnect** — close the session.

```cpp
chronolog::Client client(portalConf, queryConf);
client.Connect();

client.CreateChronicle("MyChronicle");

auto [ret, handle] = client.AcquireStory("MyChronicle", "MyStory");
