...
The configuration system is split into a lightweight loader and a set of per-component configuration classes. The loader parses the JSON file once and hands the relevant subtree to each component, which builds its own typed configuration object.
