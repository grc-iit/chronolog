...
ChronoLog provides two complementary event consumption mechanisms:
- **On-Demand Tail Read (`StoryHandle::playback`)**: Directly queries the in-memory `KeeperChunkRetentionStore` on ChronoKeeper compute nodes for the most recent $N$ events with millisecond-scale latency (~0.4 ms median, 1–5 ms at p90–p99), bypassing ChronoPlayer and HDF5 storage entirely. See [On-Demand Tail Read](./on-demand-tail-read.md).
- **Historical Story Replay (`Client::ReplayStory`)**: Queries **ChronoPlayer** for events across an arbitrary time range `[start, end)`. ChronoPlayer reads the part of the range that is already persisted from the HDF5 archives and fetches the rest from the story's ChronoKeepers (see [Durable Chunk Retention](./durable-retention.md#replay)).
...
| **Player Recording Service**                | The RPC service that receives StoryChunks pushed by ChronoKeepers configured with `dual_endpoint_rdma_extractor`. The shipped configuration no longer pushes to ChronoPlayer: replay fetches recent events from the keepers on demand. |
| **Player Data Store**                       | Tracks the active stories and the ChronoKeepers each one is recorded on (the roster ChronoVisor sends at story start), which replay queries for recent events. When chunks are pushed to ChronoPlayer it also merges them the way ChronoGrapher does. |
| **Playback/Replay Service**                 | Listens to replay queries from client applications and serves responses. It fetches the story's recent events from its ChronoKeepers (`story_range_fetch`), splits the range at the highest persisted watermark the keepers report, and reads the part below that from the HDF5 archives; see [Durable Chunk Retention](./durable-retention.md#replay). Returns all sorted events for the story within the requested range. |
