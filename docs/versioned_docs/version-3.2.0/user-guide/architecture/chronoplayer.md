...
| **Playback/Replay Service**                 | Listens to replay queries from client applications and serves responses. It fetches the story's recent events from its ChronoKeepers (`story_range_fetch`), splits the range at the oldest event the keepers still hold, and reads the part below that from the HDF5 archives. Returns all sorted events for the story within the requested range. |
