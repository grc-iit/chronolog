...
- **On-Demand Tail Read (`StoryHandle::playback`)**: Directly queries the in-memory `KeeperTailStore` on ChronoKeeper compute nodes for the most recent $N$ events with millisecond-scale latency (~0.4 ms median, 1–5 ms at p90–p99), bypassing ChronoPlayer and HDF5 storage entirely. See [On-Demand Tail Read](./on-demand-tail-read.md).
- **Historical Story Replay (`Client::ReplayStory`)**: Queries **ChronoPlayer** for historical events across an arbitrary time range `[start, end)`, serving responses from both active in-memory chunks and persistent HDF5 archives.
