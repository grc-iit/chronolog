...
| **Keeper Recording Service**    | Listens to incoming streams of log events from client applications (`record_event`), serves on-demand tail reads directly from keeper memory via `tail_get_sequences` and `tail_get_events`, and serves ChronoPlayer's replay range fetches (`story_range_fetch`). |
| **Keeper Chunk Retention Store (`KeeperChunkRetentionStore`)** | Owns every sealed StoryChunk until ChronoGrapher confirms it is written to HDF5, and indexes the most recent sealed events per story to serve `playback(N)` from memory with no payload copy. See [Durable Chunk Retention](./durable-retention.md). |
| **Ingestion Queue**             | A collection of IngestionHandles for actively recorded stories the ChronoKeeper is expecting events for. Receives log events from the Keeper Recording Service and attributes them to the appropriate Story Ingestion Handles. Also recovers late orphaned events for retired stories before archival.                                                                                                                                            |
| **StoryChunk Extraction Queue** | A mutex-protected deque of StoryChunk pointers, serving as the communication boundary between the Chunk Retention Store and the ExtractionModule. |
| **ExtractionModule**            | A configurable pipeline of extractors that consumes retired StoryChunks from the extraction queue and forwards each chunk through every extractor in the chain. The module owns a fixed-size pool of extraction-stream threads (`extraction_stream_count`) that drain the queue in parallel. Available extractor types: **`csv_extractor`** writes chunks to a local POSIX directory; **`single_endpoint_rdma_extractor`** drains chunks to a single RDMA endpoint (typically a ChronoGrapher); **`dual_endpoint_rdma_extractor`** fans chunks out to two RDMA endpoints simultaneously (typically a ChronoGrapher and a ChronoPlayer). The chain is assembled from JSON at startup — see [Server Configuration → ExtractionModule](../configuration/server-configuration.md#extractionmodule). |
...
| **DataStoreAdminService**       | Listens to Start/Stop Story recording notifications from ChronoVisor, which trigger instantiation or dismantling of the appropriate StoryPipelines, and to ChronoGrapher's persisted-watermark reports (`report_story_watermarks`), which let the retention store free persisted chunks. |
...
For full architectural details on zero-copy indexing, capacity bounds, and live unsealed reads, see [On-Demand Tail Read Architecture](./on-demand-tail-read.md). How sealed chunks are kept until they are persisted is described in [Durable Chunk Retention](./durable-retention.md).
