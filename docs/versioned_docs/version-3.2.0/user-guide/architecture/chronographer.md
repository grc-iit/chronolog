...
<svg viewBox="0 0 720 650" width="100%" xmlns="http://www.w3.org/2000/svg" fontFamily="system-ui, sans-serif">
  ...
  {/* Bottom annotation */}
  <text x="440" y="586" textAnchor="middle" fill="#9ca3b0" fontSize="7" fontStyle="italic">Complete StoryChunks are archived in the HDF5 archive</text>
...
| **StoryChunk Extraction Queue**       | Provides a thread-safe communication boundary between the Grapher Data Store and the ExtractionModule as retired StoryChunks leave their StoryPipelines and wait to be picked up by an extraction stream.                                                                         |
| **ExtractionModule**                  | A configurable pipeline of extractors that consumes retired StoryChunks from the extraction queue and forwards each chunk through every extractor in the chain. The module owns a fixed-size pool of extraction-stream threads (`extraction_stream_count`) that drain the queue in parallel. The default ChronoGrapher chain contains a single **`hdf5_extractor`** that serializes StoryChunks to the HDF5 archive under `hdf5_archive_dir`. The chain must include `hdf5_extractor`, the only extractor that confirms chunks written so that keepers can free them; other types (`csv_extractor`, `single_endpoint_rdma_extractor`) may run alongside it, and a chain without it is rejected at startup. The chain is assembled from JSON at startup — see [Server Configuration → ExtractionModule](../configuration/server-configuration.md#extractionmodule). |
| **Watermark Report Publisher**      | Tracks, per story, how far the story is written to HDF5 without gaps (the persisted watermark) and sends it every `watermark_report_interval_secs` to the ChronoKeepers that contributed chunks to the story, so they can free them. See [Durable Chunk Retention](./durable-retention.md). |
