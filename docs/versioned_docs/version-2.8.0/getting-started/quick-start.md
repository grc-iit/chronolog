...
Download the pre-built binary tarball for v2.8.0 from the [ChronoLog GitHub Releases](https://github.com/grc-iit/ChronoLog/releases) page:

```bash
wget https://github.com/grc-iit/ChronoLog/releases/download/v2.8.0/chronolog-2.8.0-linux-x86_64.tar.gz
```
...
```bash
tar -xzf chronolog-2.8.0-linux-x86_64.tar.gz
```
...
- `tools/cli/chrono-client-cli` — interactive CLI tool
- `tools/benchmark/chrono-bench` — scripted runs and performance test
...
From inside the extracted directory (`chronolog-2.8.0`), run the deployment script:

```bash
tools/deploy/deploy_local.sh --start
```
...
Download the RPM package for v2.8.0 from the [ChronoLog GitHub Releases](https://github.com/grc-iit/ChronoLog/releases/tag/v2.8.0) page:

```bash
wget https://github.com/grc-iit/ChronoLog/releases/download/v2.8.0/chronolog-2.8.0-linux-x86_64.rpm
```
...
sudo dnf install ./chronolog-2.8.0-linux-x86_64.rpm

# Or with yum (RHEL 7, CentOS)
sudo yum install ./chronolog-2.8.0-linux-x86_64.rpm
```
...
```bash
/usr/share/chronolog/tools/deploy/deploy_local.sh --start
```
...
```bash
/usr/share/chronolog/tools/deploy/deploy_local.sh --stop
```

To also remove generated logs, configuration artifacts and stored data:

```bash
/usr/share/chronolog/tools/deploy/deploy_local.sh --clean
```
...
Download the DEB package for v2.8.0 from the [ChronoLog GitHub Releases](https://github.com/grc-iit/ChronoLog/releases/tag/v2.8.0) page:

```bash
wget https://github.com/grc-iit/ChronoLog/releases/download/v2.8.0/chronolog-2.8.0-linux-x86_64.deb
```
...
```bash
sudo apt install ./chronolog-2.8.0-linux-x86_64.deb
```
...
```bash
~/chronolog-install/chronolog/tools/deploy/deploy_local.sh --start
```
...
```bash
~/chronolog-install/chronolog/tools/deploy/deploy_local.sh --stop
```

To also remove generated logs, configuration artifacts and stored data:

```bash
~/chronolog-install/chronolog/tools/deploy/deploy_local.sh --clean
```
...
```bash
docker pull ghcr.io/grc-iit/chronolog:v2.8.0
```

## Run the Container

```bash
docker run -it --rm ghcr.io/grc-iit/chronolog:v2.8.0 bash
```
