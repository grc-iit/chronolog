...
<svg viewBox="0 0 720 640" width="100%" xmlns="http://www.w3.org/2000/svg" fontFamily="system-ui, sans-serif">
  ...
  <text x="100" y="147" textAnchor="middle" fill="#9ca3b0" fontSize="6" fontStyle="italic">(stub in v2.8.0)</text>
...
| ----------------------------------------| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Client Portal RPC Service**          | Manages communication sessions between client applications and ChronoVisor using a Thallium RPC engine configured for efficient concurrent connections. The Connect handshake carries a `protocol_version` field that the Visor matches against its own expected version; mismatches are rejected with `CL_ERR_PROTOCOL_VERSION_MISMATCH` so wire-level skew surfaces as a clean error rather than silent deserialization failures.                                                                                                          |
| **Client Authentication Module**       | Defines the authentication and authorization interface for client access control. In v2.8.0 all checks are implemented as stubs that grant access unconditionally; full role-based authentication is planned for a future release.                                                                                                                                                                                                                                                                                                           |
