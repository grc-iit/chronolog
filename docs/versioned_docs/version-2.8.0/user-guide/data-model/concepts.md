...
Stories expose configurable attributes for indexing granularity, type, tiering policy, and access permission. In ChronoLog v2.8.0, these Story-level attributes are accepted but do not affect runtime behavior yet.
...
An Archive provides historical storage associated with a Chronicle. Each Archive is identified by an `ArchiveId`, computed as `CityHash64(toString(chronicleId) + archiveName)`. Archives carry a user-defined property list for application-specific metadata. The list has no effect so far in ChronoLog v2.8.0.
