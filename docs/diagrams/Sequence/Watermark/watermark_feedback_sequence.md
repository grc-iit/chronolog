...
The cross-component view of durable chunk retention. Chunk retention on the keeper is **durability-gated** by the grapher's persisted watermark `W`, upholding the invariant **`E ≤ W`** — a keeper never frees a chunk whose events are not confirmed durable in HDF5. The player then splits replay at **`B = min(hot_floor)`** across the story's keeper roster: everything below `B` is guaranteed archived, so the cold (archive) read stops at `B` and the hot side comes from the keepers on demand (`story_range_fetch`). Duplicates from re-sends/overlap are cleaned by read-side `EventSequence` dedup. The user-facing description is `docs/versioned_docs/version-3.2.0/user-guide/architecture/durable-retention.md`. Fonts enlarged ~50%.
