...
void signalHandler(int sig)
{
    ...
    if(agent_ptr)
        agent_ptr->shutdown(), delete agent_ptr;
    std::exit(sig);
}
...
int main(int argc, char** argv)
{
    ...
    if(conf_file.empty())
    {
        std::cerr << "Usage: " << argv[0] << " -c <config.json>" << " [-C chronicle] [-S story]"
    ...
    int r = chronolog::chrono_monitor::initialize(conf.CLIENT_CONF.CLIENT_LOG_CONF.LOGTYPE,
    ...
    std::cout << "Reading [" << start_time << "," << end_time << "] from " << chronicle_name << "." << story_name
              << "\n";
    ...
    for(auto* chunk: list_of_chunks)
    {
        ...
        for(auto const& kv: *chunk)
        {
            ...
            std::cout << "  storyId=" << e.storyId << ", time=" << e.eventTime << ", clientId=" << e.clientId
                      << ", index=" << e.eventIndex << ", record=\"" << e.logRecord << "\"\n";
        }
        delete chunk;
    }
    ...
    agent_ptr
            ->shutdown(); // joins the monitoring thread :contentReference[oaicite:0]{index=0}:contentReference[oaicite:1]{index=1}
    delete agent_ptr;
    agent_ptr = nullptr;
