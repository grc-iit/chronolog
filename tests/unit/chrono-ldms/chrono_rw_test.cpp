...
int main(int argc, char** argv)
{
    if(argc < 6)
    {
        std::cerr << "args: conf chronicle story n hold_secs\n";
        return 2;
    }
    ...
    chronolog::ClientQueryServiceConf q;
    q.PROTO_CONF = cm.QUERY_CONF.PROTO_CONF;
    q.IP = cm.QUERY_CONF.IP;
    q.PORT = cm.QUERY_CONF.PORT;
    q.PROVIDER_ID = cm.QUERY_CONF.PROVIDER_ID;
    ...
    {
        std::cerr << "acquire failed " << acq.first << "\n";
        return 1;
    }
    ...
    if(hold > 0)
    {
        std::cout << "holding story acquired for " << hold << "s...\n";
        sleep(hold);
    }
    ...
    if(!ev.empty())
        std::cout << "  first: " << ev.front().log_record().substr(0, 80) << "\n";
