...
{
	if (argc < 6) {
		fprintf(stderr, "usage: %s <client_conf> <chronicle> "
	...
	chronolog::chrono_monitor::initialize(cm.LOG_CONF.LOGTYPE, "chrono_bench.log",
		cm.LOG_CONF.LOGLEVEL, cm.LOG_CONF.LOGNAME, cm.LOG_CONF.LOGFILESIZE,
		cm.LOG_CONF.LOGFILENUM, cm.LOG_CONF.FLUSHLEVEL);

	chronolog::ClientPortalServiceConf portal;
	portal.PROTO_CONF = cm.PORTAL_CONF.PROTO_CONF; portal.IP = cm.PORTAL_CONF.IP;
	portal.PORT = cm.PORTAL_CONF.PORT; portal.PROVIDER_ID = cm.PORTAL_CONF.PROVIDER_ID;
	...
	for (int t = 0; t < threads; t++) {
		story_names[t] = "s" + std::to_string(t) + "_" +
				 std::to_string(plen) + "_" + std::to_string(getpid());
	...
	printf("chronolog,sync,%zu,%llu,%d,%.0f,%.2f,%.0f,%.2f,%llu,%llu,%.6f,%.6f\n",
	       plen, (unsigned long long)n, threads,
	       acked_eps, mbps, acked_eps, mbps,
	       (unsigned long long)(n - errors.load()),
	       (unsigned long long)errors.load(), t0, t1);
	fprintf(stderr, "[chronolog] size=%zu thr=%d | acked %.0f ev/s (%.1f MB/s) | "
		"errors=%llu | %.1f MB in %.2fs\n",
		plen, threads, acked_eps, mbps,
		(unsigned long long)errors.load(),
		(double)n * plen / (1024*1024), t1 - t0);

	for (int t = 0; t < threads; t++)
		client.ReleaseStory(chronicle, story_names[t]);
	client.Disconnect();
	return 0;
}
