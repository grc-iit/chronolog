...
int main(int argc, char** argv)
{
    ...
    {
        fprintf(stderr,
                "usage: %s <brokers> <topic> <payload_size> "
    ...
    if(!rkt)
    {
        fprintf(stderr, "topic_new failed\n");
        return 1;
    }
    ...
    printf("%s,%s,%zu,%llu,%d,%.0f,%.2f,%.0f,%.2f,%llu,%llu,%.6f,%.6f\n",
           ...
           threads,
           enqueue_eps,
           enqueue_eps * plen / (1024 * 1024),
           acked_eps,
           acked_eps * plen / (1024 * 1024),
           (unsigned long long)g_delivered.load(),
           (unsigned long long)g_errors.load(),
           t0,
           t2);
    fprintf(stderr,
            "[%s] size=%zu thr=%d acks=%s | enqueue %.0f ev/s (%.1f MB/s) | "
